...
const DispensationPage = () => {
  const {
    ...
    exporting, downloadReceipt, printingId, printReceipt, totalPrice, unitOf, creating, saving, openCreate, closeCreate,
  } = useDispensationPage()
        ...
        <div className="purchase-list">
          ...
          <article className="purchase-card" key={purchase.id} aria-labelledby={`purchase-${purchase.id}`}>
            <div className="purchase-header">
              ...
              <button type="button" className="btn-light" disabled={printingId !== null}
                aria-label={`Imprimer la facture de l’achat ${purchase.id}`}
                onClick={() => printReceipt(purchase.items[0])}>
                {printingId === purchase.items[0].id ? 'Impression…' : 'Imprimer'}
              </button>
